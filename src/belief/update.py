...
class BeliefUpdater:
    def __init__(self, alpha: float = 1.0, beta: float = 2.0, decay: float = 1.0):
        """
        Log-odds update model.
        alpha: Weight for observation evidence.
        beta: Weight for deterministic transition evidence.
        decay: Multiplier applied to prior log-odds before new evidence arrives.
               Values below 1.0 gradually pull stale beliefs back toward uncertainty.
        """
        self.alpha = alpha
        self.beta = beta
        self.decay = decay
    ...
    def update(self, prior_prob: float, obs_prob: float = None, action_delta: float = None) -> float:
        ...
        new_l = (self.decay * l_prior) + (self.alpha * l_obs) + (self.beta * l_act)
